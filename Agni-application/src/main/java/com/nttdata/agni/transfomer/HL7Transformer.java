...
public class HL7Transformer extends AbstractTransformer {
    ...
    public  HashMap<String, String> getHL7ValuesMap(Message hapiMsg, HashMap<String, String> tempMap) throws HL7Exception{ 
    	...
    	for (Map.Entry<String, String> entry : tempMap.entrySet()) {
            ...
            if (value == "RXA-5"){
            	valueHL7 = terser.getSegment("RXA").getField(5, 0).toString();
            	System.out.println(key+"-"+value+"harendra:"+valueHL7);
            }
            dataMap.put(key, valueHL7);
            
        }
    	/*
    	Segment pidSegment = terser.getSegment("PID");
 		Type[] pid5Type = pidSegment.getField(5);
 		System.out.println("harendra:"+terser.getSegment("RXA").getField(5, 0).toString());
 		
 		for (int indx = 0; indx < pid5Type.length; indx++) {
    	String subIndex = "(" + indx + ")";
    	terser.get("PID-5" + subIndex + "-1");  // gets familyName
    	terser.get("PID-5" + subIndex + "-2");  // gets givenName
 		}
 		*/
        return dataMap;
    }
